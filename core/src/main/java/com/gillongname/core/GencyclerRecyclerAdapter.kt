 ...
 *
 * @property T The type of the data container, anything that extends [GencyclerModel]
 *
 ...
 *
 */
abstract class GencyclerRecyclerAdapter<E : GencyclerModel, VH : GencyclerHolder>(
	 ...
	 */
	@JvmName("addOperator")
	operator fun plus(element: T): Unit = add(element)
	 ...
	 */
	@JvmName("addOperator")
	operator fun plus(elements: List<T>): Unit = add(elements)
	 ...
	 */
	@JvmName("addOperator")
	operator fun minus(range: IntRange): Boolean = remove(range)
	 ...
	 */
	@JvmName("addOperator")
	operator fun minus(elements: List<T>): Boolean = remove(elements)
	 ...
	 */
	@JvmName("addOperator")
	operator fun minus(element: T): Boolean = remove(element)
	 ...
	 */
	@JvmName("addOperator")
	operator fun minus(position: Int): Boolean = remove(position)
	...
	/**
	 * Swap the item at [oldPosition] with the item at [newPosition]
	 *
	 * @param oldPosition index of the element to replace
	 * @param newPosition index of where the item will move to
	 *
	 * If updateUi is true then we will call [notifyItemMoved]
	 *
	 */
	open fun swap(oldPosition: Int, newPosition: Int): Boolean {
		if (oldPosition < 0 || oldPosition > size ||
				newPosition < 0 || newPosition > size) return false

		val temp = elements[oldPosition]
		elements[oldPosition] = elements[newPosition]
		elements[newPosition] = temp

		if (!updateUi) return true

		if (isUiThread()) {
			notifyItemMoved(oldPosition, newPosition)
		} else throw uiThreadException
