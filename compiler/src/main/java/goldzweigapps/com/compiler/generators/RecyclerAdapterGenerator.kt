...
class RecyclerAdapterGenerator(private val rClass: ClassName) {
    ...
    private val recyclerAdapterSuperClass = ClassName(
        Packages.GENCYCLER,
    ...
    private val filterableClassName = ClassName(
        Packages.ANDROID_WIDGET,
        "Filterable"
    )

    private var androidFilterClassName = ClassName(
        Packages.ANDROID_WIDGET,
        "Filter"
    )
    ...
    fun generate(adapter: Adapter): FileSpec {
        ...
        adapter.listenerInterface?.let {
            ...
            actionListenerClassName = ClassName(
                "${adapter.packageName}.${adapter.name}",
                "ActionsListener"
            )
        }
        ...
        adapterConstructor
            .addParameter(
                ParameterSpec.builder(Names.UPDATE_UI, BOOLEAN)
                    .defaultValue("true")
                    .build()
            )
    ...
    private fun generateCreateViewHolderFunc(
        ...
        return if (viewType != null) {
            with(createViewHolderFunctionBuilder) {
                ...
                if (viewType.actions.isEmpty()) {
                    addStatement(
                        "return %T(inflate(%T.layout.%L, parent))",
                    ...
                    for (action in viewType.actions) {
                        when (action) {
                            ...
                            Actions.CLICK ->
                                createActionListenerValueAction(
                                    "Clicked",
            ...
            with(createViewHolderFunctionBuilder) {
                ...
                addStatement(
                    "val viewHolderType = %T.valueOf(viewType)",
    ...
    private fun generateBindViewHolderFunc(
        viewType: ViewType? = null,
        codeBlockBuilder: CodeBlock.Builder? = null
    ): FunSpec {
    ...
    private fun generateRecycleViewHolderFunc(
        ...
        return if (viewHolder != null) {
            recycleViewHolderFuncBuilder
                ...
                .addStatement(
                    "%L%L(holder, holder.adapterPosition)",
    ...
    private fun generateItemViewTypeFunc(
    ...
    private fun generateAbstractBindFunc(viewType: ViewType): FunSpec {
        ...
        return FunSpec.builder("${Methods.ABSTRACT_BIND_CUSTOM}${viewHolder.simpleName}")
            .addKdoc(
                KDocs.ABSTRACT_BIND_FUNC, viewHolder,
    ...
    private fun generateAbstractPerformFilterFunc(typeName: TypeName): FunSpec {
        ...
        return FunSpec.builder(Methods.PERFORM_FILTER)
            .addKdoc(
                KDocs.ABSTRACT_PERFORM_FILTER_FUNC, dataTypeParamName,
    ...
    private fun CodeBlock.Builder.insertCreateViewHolderCase(
        viewType: ViewType,
        viewHolderEnumName: String
    ): CodeBlock.Builder {
    ...
    private fun CodeBlock.Builder.insertBindViewHolderCase(
        viewType: ViewType,
        viewHolderSimpleName: String
    ): CodeBlock.Builder {

        return addStatement("is %T ->", viewType.viewHolderType)
            .indent()
            .addStatement(
                "%L%L(holder, %L[%L] as %T, position)",
                Methods.ABSTRACT_BIND_CUSTOM,
    ...
    private fun CodeBlock.Builder.insertRecycleViewHolderCase(
        viewType: ViewType,
        viewHolderSimpleName: String
    ): CodeBlock.Builder {
    ...
    private fun CodeBlock.Builder.insertItemViewTypeCase(
        viewType: ViewType,
        viewHolderEnumName: String
    ): CodeBlock.Builder {
    ...
    private fun CodeBlock.Builder.insertExceptionThrowCase(
        showPositionLine: Boolean = false,
        viewTypesSimpleNames: List<String>? = null
    ): CodeBlock.Builder {
    ...
    private fun CodeBlock.Builder.createActionListenerValueAction(
        action: String,
    ...
    private fun FunSpec.Builder.createActionListenerValueAction(
        action: String,
        holderSimpleName: String,
        params: String,
        actionParamaters: String? = null
